...
#droids <- starwars %>% filter(species == "Droids")
#plot(droids$height, droids$mass)

#starwars %>% filter(species == "Droids") %>% plot(mass ~ height, data = .)
```
...
```{r}
#filter(starwars, sex= 'male', max(height))
